            ...
            <section class="modal-card-body " style="margin-bottom: inherit;">
                <div class="columns" style="margin-bottom: inherit;">
                    ...
                    <div class="column is-5">
                        ...
                        <GoogleLogin style="margin-bottom: 0.75rem;"
                            :params="params"
                            :renderParams="renderParams"
                            :onSuccess="loginGoogle"
                            :onFailure="oauthError">
                        </GoogleLogin>
                        <a v-on:click="loginFacebook()" href="#" class="fb"><div class="connect"></div>Sign in with Facebook</a>
                        <!-- <div v-on:click="loginFacebook()" class="fb-login-button" data-width="240" data-size="large" data-button-type="login_with" data-auto-logout-link="false" data-use-continue-as="true"></div> -->
                    </div>
                </div>
                ...
                <div class="notification is-danger" v-if="loginStatus === 3">
...
const   GOOGLE_ERROR = 3;
const   ALREADY_CONNECTED = 4;
...
export default {
  ...
  methods: {
      ...
      loginGoogle(googleUser) {
            ...
            this.axios.post(`http://localhost:8080/register/tpa?id_tpa=${id}/`)
            .then((response) => {
                if (response.data === "Token doesn't exist.") {
                    this.loginStatus = ERROR_ARGS;
                    return;
                } else {
                    localStorage.setItem("authToken", response.data);
      ...
      loginFacebook() {
          var closeModal = this.close;
          var loginState = this.loginStatus;
          var loginResponse;

          Vue.FB.getLoginStatus(function (response) {
              if (response.status === "connected") {
                  loginState = ALREADY_CONNECTED;
                  loginResponse = response;
              }
          });
          if (loginState === ALREADY_CONNECTED) {
                this.axios.post(`http://localhost:8080/register/tpa?id_tpa=${loginResponse.authResponse.userID}/`)
                .then((response) => {
                    if (response.data === "Token doesn't exist.") {
                        this.loginStatus = ERROR_ARGS;
                        return;
          ...
          Vue.FB.login(function (response) {
              if (response.status === "connected") {
                this.axios.post(`http://localhost:8080/register/tpa?id_tpa=${response.authResponse.userID}/`)
                .then((response) => {
                    if (response.data === "Token doesn't exist.") {
                        this.loginStatus = ERROR_ARGS;
                        return;
                  ...
                  loginState = GOOGLE_ERROR;
              }
          });
      },
      oauthError() {
          this.loginStatus = GOOGLE_ERROR;
      }
  }
}
</script>
